...
class PPOTrainer:
    ...
    def compute_action(self, obs, deterministic=False):
        ...
        if self.discrete:  # Please use categorical distribution.
            ...
            # Fix to the reward drop problem at 85% train. 
            # Since this is log likelihood we can say the best log likelihood is 0 (or e^0 = 1 = 100%)
            # So we clip it to that, and that fixed the issue.
            # action_log_probs = torch.clip(action_log_probs,max=0)
            #?
            #------------------------------------
    ...
    def compute_loss(self, sample):
        ...
        s1 = ratio * adv_targ
        s2 = torch.clamp(ratio,1-self.clip_param, 1+self.clip_param)*adv_targ
        ...
        # value_loss = None
        # pass
        value_loss = torch.nn.functional.mse_loss(return_batch,values)#0.5 * (return_batch - values).pow(2)
